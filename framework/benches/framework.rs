...
struct UserData {
    tasks: HashMap<String, TaskHandler<UserData>>,
}
...
fn large_dispatch(c: &mut Criterion) {
    ...
    c.bench_function("full framework", |b| {
        let bench_ctx = ctx.clone();
        let bench_registry = registry.clone();

        b.to_async(Runtime::new().expect("error creating runtime"))
            .iter(move || {
                let local_ctx = bench_ctx.clone();
                let local_registry = bench_registry.clone();
                ...
                let discord_event = twilight_gateway::parse(event.payload, EventTypeFlags::all())
...
criterion_group!(benches, large_dispatch);
